...
void octree::compute_properties(tree_structure &tree) {
  ...
  //TODO maybe define this sometime earlier to prevent (small) slowdowns bcause of eventcreate
  my_dev::dev_mem<double4> multipoleD(devContext);
  my_dev::dev_mem<real4>   nodeLowerBounds(devContext); //Lower bounds used for computing boxsizes
  my_dev::dev_mem<real4>   nodeUpperBounds(devContext); //Upper bounds used for computing boxsizes
...
//  //the group center since now we do not require a sort operation to construct the groups.
...
//    LOGF(stderr, "Compute properties took: %lg  wait: %lg \n", get_time()-t0, get_time()-t1);
  ...
  int grpTree_endGrp;
  build_GroupTree(tree.n_groups, &grpKeys[0],
                  nodes, nodeKeys, node_levels,
                  grpTree_n_levels, grpTree_n_nodes, grpTree_startGrp, grpTree_endGrp);
  ...
  computeProps_GroupTree(&tree.groupCenterInfo[0], &tree.groupSizeInfo[0],
      localGrpTreeCntSize, &localGrpTreeCntSize[grpTree_n_nodes], nodes, node_levels, grpTree_n_levels);
