...
void trBeginTile(TRcontext *tr)
{
   ...
   fprintf(stderr, "View was: %d %d %d %d || wordt: %d %d %d %d \n",
          tr->ViewportSave[0], tr->ViewportSave[1],tr->ViewportSave[2],
          tr->ViewportSave[3], 0,0,tileWidth,tileHeight);
   ...
   fprintf(stderr, "L R B T Near Far: %f %f %f %f %f %f \n",
              left, right, bottom, top, tr->Near, tr->Far);
...
int trEndTile(TRcontext *tr)
{
   ...
   if (tr->TileBuffer) {
	 ...
	 // glPixelStorei(GL_PACK_ALIGNMENT,1);
	  ...
	  printf("Test srcx %d srcy %d width: %d height: %d  format: %d type: %d buffer: %p \n",
		  srcX, srcY, srcWidth, srcHeight, tr->TileFormat, tr->TileType, tr->TileBuffer);
   }
   ...
   if (tr->CurrentTile >= tr->Rows * tr->Columns) {
     fprintf(stderr, "Restoring ! \n");
      ...
      if (tr->Perspective)
      {
        glMatrixMode(GL_PROJECTION);
        glLoadIdentity();
        glFrustum(tr->Left, tr->Right, tr->Bottom, tr->Top, tr->Near, tr->Far);
      }
      return 0;
   }
   else
