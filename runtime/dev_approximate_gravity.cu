...
#else
        int v0 = inclusive_scan_block<DIM2>(prefix_shmem, nchild, tid);        // inclusive scan to compute memory offset of each child
        prefix[tid] = v0;
        __syncthreads();
#endif
...
#else
        v0 = inclusive_scan_block<DIM2>(prefix_shmem, 1 - (split || !use_node), tid);
        prefix[tid] = v0;
        __syncthreads();
#endif
...
#else
        v0 = inclusive_scan_block<DIM2>(prefix_shmem, (int)flag, tid);       // inclusive scan on flags to construct array
        prefix0[tid] = v0;
        __syncthreads();
#endif
#endif
...
#else
        v0 = inclusive_scan_block<DIM2>(prefix_shmem, nbody, tid);        // inclusive scan to compute memory offset for each body
        prefix0[tid] = v0;
        __syncthreads();
#endif
