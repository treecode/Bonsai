...
#define ADDUSAGE(line) {{std::stringstream oss; oss << line; opt.addUsage(oss.str());}}
	...
	const float sizeRatio = 300.0/200.0;  /* size(M31)/size(MW) */
	const float massRatio =   7.1/5.8;    /* mass(M31)/mass(MW) */
	const float Rsep      = 778.0/Runit;  
	const float Rp        = 20.0/Runit;     /* page 18 in astro-ph/9509010 */

	const float lMW       = to_rad(-90.0);
	const float bMW       = to_rad(0.0);
...
#if 0	
	const float Zcrd = Rsep * std::sin(bR);
	const float Xcrd = Rsep * std::cos(bR) * std::cos(lR);
	const float Ycrd = Rsep * std::cos(bR) * std::sin(lR);

	const float Vr_x = V1 * std::cos(bR);
	const float Vr_y = V1 * std::sin(bR);
#endif
	...
	if( !(rv1 = fopen(mw_fname.c_str(),"rb")) ) 
	{
		fprintf(stderr,"Can't find Milky Way galaxy file  %s\n", mw_fname.c_str());
		exit(0);
	}
	if( !(rv2 = fopen(m31_fname.c_str(),"rb")) ) 
	{
	...
	FILE *outfile;
	if( !(outfile = fopen(out_fname.c_str(),"wb")) ) 
	{
	...
	sprintf(settingsFile, "%s.settings", out_fname.c_str());
